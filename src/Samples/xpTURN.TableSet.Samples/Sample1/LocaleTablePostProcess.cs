...
namespace Samples
{
    [TableSetPostProcess(typeof(Sample1TableSet), 100)]
    public class LocaleTablePostProcess : TableSetPostProcess
    {
        ...
        {
            //
            var sample1TableSet = tableSet as Sample1TableSet;
            if (sample1TableSet == null)
                return;
            ...
            sample1TableSet.AddTable(nameof(LocaleDataTable), localeDataTable);

            // Remove the original TranslatedDataTable
            sample1TableSet.DelTable(nameof(TranslatedDataTable));

            // Update the TableId to the new LocaleDataTable
            var translatedDataTableId = sample1TableSet.GetTableId(nameof(TranslatedDataTable));
            var localeDataTableId = sample1TableSet.GetTableId(nameof(LocaleDataTable));
            foreach (var keyValuePair in sample1TableSet.GetMetaDataTable().MapAliasData)
            {
