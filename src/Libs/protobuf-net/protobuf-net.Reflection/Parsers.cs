    ...
    public partial class DescriptorProto : ISchemaObject, IType, IMessage, IReserved<DescriptorProto.ReservedRange, FieldDescriptorProto>
    {
        ...
        private void ParseMap(ParserContext ctx)
        {
            ...
            switch (keyType)
            {
                case 0:
#if XPTURN
                    // During parsing, it is not possible to determine whether it is a user-defined Enum type.
                    // so it is checked after parsing is complete in FieldDescriptorProto.PostprocessParseSchema
                    break;
#endif
...
#if XPTURN
        internal void ResolveMapEntry(ParserContext ctx)
        {
            var tokens = ctx.Tokens;
            foreach (var msgType in MessageTypes)
            {
                foreach (var nestedType in msgType.NestedTypes)
                {
                    var isMap = nestedType?.Options?.MapEntry ?? false;
                    if (isMap)
                    {
                        FieldDescriptorProto keyDesc = nestedType.Fields.Single(x => x.Number == 1);
                        if (keyDesc.type == default && !string.IsNullOrEmpty(keyDesc.TypeName))
                        {
                            if (!TryResolveEnum(keyDesc.TypeName, null, out EnumDescriptorProto _enumDesc, true))
                            {
                                ctx.Errors.Error(tokens.Previous, $"map key type is not specified; '{keyDesc.TypeName}'", ErrorCode.InvalidMapKeyType);
                                continue;
                            }

                            keyDesc.type = FieldDescriptorProto.Type.TypeEnum;
                        }
                    }
                }
            }
        ...
        internal static bool TryIdentifyType(string typeName, out Type type)
        {
            ...
            switch (typeName)
            {
                ...
                default:
                    ...
                    return false;
#if XPTURN
                case "DateTime": return Assign(Type.TypeDateTime, out @type);
                case "TimeSpan": return Assign(Type.TypeTimeSpan, out @type);
                case "Uri": return Assign(Type.TypeUri, out @type);
                case "Guid": return Assign(Type.TypeGuid, out @type);
#endif
            }
        }
